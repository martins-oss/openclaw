...
const MAX_IMAGE_DESCRIPTION_TIMEOUT_MS = 30_000;

function normalizeImageDescriptionTimeoutMs(timeoutMs: number | undefined) {
  if (typeof timeoutMs !== "number" || !Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    return undefined;
  }
  const milliseconds = timeoutMs < 1_000 ? timeoutMs * 1_000 : timeoutMs;
  return Math.min(MAX_IMAGE_DESCRIPTION_TIMEOUT_MS, Math.max(1_000, Math.floor(milliseconds)));
}

function resolveImageDescriptionTimeoutMs(timeoutMs: number | undefined, startedAtMs: number) {
  const normalizedTimeoutMs = normalizeImageDescriptionTimeoutMs(timeoutMs);
  if (normalizedTimeoutMs === undefined) {
    return undefined;
  }
  return Math.max(1_000, Math.floor(normalizedTimeoutMs - (Date.now() - startedAtMs)));
}
