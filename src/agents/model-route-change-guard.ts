...
function describeEscalationReason(params: {
  ...
  if (params.activeBilling === "metered" && params.selectedProvider !== params.activeProvider) {
    return `provider changed within metered billing (${params.selectedProvider} → ${params.activeProvider})`;
  }
  if (params.activeBilling === "metered" && params.selectedAuthMode !== params.activeAuthMode) {
    return `auth profile changed within metered billing (${params.selectedAuthMode} → ${params.activeAuthMode})`;
  }
  return undefined;
}
