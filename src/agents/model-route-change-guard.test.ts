...
describe("model-route-change-guard", () => {
  ...
  it("describes provider changes that stay inside metered billing accurately", () => {
    const result = evaluateModelRouteChangeGuard({
    ...
    expect(result.action).toBe("escalate");
    expect(result.reason).toBe("provider changed within metered billing (openai → openrouter)");
  });

  it("describes auth-profile changes that stay inside metered billing accurately", () => {
    const result = evaluateModelRouteChangeGuard({
      ...
      selectedAuthMode: "api-key",
      activeAuthMode: "aws-sdk",
    });

    expect(result.action).toBe("escalate");
    expect(result.reason).toBe("auth profile changed within metered billing (api-key → aws-sdk)");
  });
